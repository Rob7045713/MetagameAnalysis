...
class DTree(object):
    ...
    """
    def __init__(self, traindat, label_name, weights=None):
        """
        Arguments:
        ...
        if weights is None:
            self.weights = np.array([1] * traindat.shape[0])
...
def get_tree(df, label_name, maxdepth=10, verbose=0):
    ...
    w = np.array([1] * df.shape[0])

    dt=DTree(train_df,label_name, weights = w)
    dt.train(maxdepth, verbose=verbose)
    dt.tune(tune_df)
    return dt

def w_get_tree(df, label_name, w, maxdepth=10, verbose=0):
    """
    Get a decision tuned decision tree. Tuning is done on a held-aside tuning set with 20% of the data.
    ...
    train_df.index=np.arange(ntrain)
    tune_df.index=np.arange(nexamples-ntrain)
    dt=DTree(train_df,label_name, w)
    dt.train(maxdepth, verbose=verbose)
    dt.tune(tune_df)
    return dt